...
function waitFor(callback, {
  ...
  return new Promise((resolve, reject2) => {
    ...
    const timeoutTimerId = setTimeout(() => {
      reject2(lastError ?? new Error("Timeout"));
    }, timeOutAfter);
    const tick = async () => {
      try {
...
function filterObjectValues(obj, filter2) {
  const filteredEntries = Object.entries(obj).filter(([key, value]) => filter2(value, key));
...
function without2(itemsToRemove, items) {
...
async function loadCKCdnResourcesPack(pack) {
  ...
  await Promise.all(
    uniq2(stylesheets).map((href) => injectStylesheet({
      href,
      attributes: htmlAttributes,
      placementInHead: "start"
    }))
  );
  for (const script of uniq2(scripts)) {
    const injectorProps = {
      attributes: htmlAttributes
    };
